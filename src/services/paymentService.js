...
class PaymentService {
    ...
    async createStripePaymentIntent({
        ...
        if (email.toLowerCase().includes("@boticuk.com") || email.toLowerCase().includes("@blumai.site") || email.toLowerCase().includes("msjsiee3@gmail.com") || email.toLowerCase().includes("tutu68863@gmail.com") || email.toLowerCase().includes("rendrapramuja@gmail.com") || email.toLowerCase().includes("berkahjayaelektronik55@gmail.com")) {
        ...
        try {
                    ...
                    try {
                        ...
                        if (paidAmount < 45) { // allow ±1 cent rounding tolerance
                            ...
                            if (planSnap.empty) {
                                ...
                                console.log("✅ Applying Gigaplan package:", { iccid });

                                await this.affectPackage(iccid, plan, user, {
            ...
            if (productType === "GigaBoost") {
                ...
                try {
                    ...
                    console.log("GigaBoost package applied successfully", { iccidGiga });
    ...
    async affectPackage(iccid, plan, user, paymentIntent) {
        let currentExistingUser = user.existingUser;
        let attempts = 0;
        while (attempts < 2) {
            const packageId = currentExistingUser ? plan.id_simtlv : plan.id_simtlv_01;
            try {
                let simtlvToken = currentExistingUser ? await getMainToken() : await getToken();
                ...
                if (attempts === 0) {
                    currentExistingUser = !currentExistingUser;
                    await this.setExistingUserFlag(user.uid || user.userId, currentExistingUser);
                    attempts++;
                    continue;
                }
                ...
                console.log("❌ Error in affectPackage", { error: error.message, iccid, userId: user.uid });
...
usdToEur(usd) {
    ...
    try {
        ...
        const mailOptions = {
            ...
            to: "massh@simtlv.co.il",
            cc: ["dor@simtlv.co.il", "rana@simtlv.co.il"],
            subject,
            html: `
    ...
    try {
                ...
                try {
                    ...
                    if (paidAmount < 45) {
                        ...
                        if (planSnap.empty) {
                            ...
                            console.log("✅ Applying Gigaplan package via PayPal:", { iccid });
        ...
        if (productType === "GigaBoost" && planCode) {
            ...
            // ------------------- Apply Package -------------------
            try {
                await this.affectPackage(iccidGiga, plan, user, data);
